...
import std.file : exists, mkdirRecurse, read, readText, remove, rmdirRecurse,
    tempDir, write;
...
private JSONValue metric(string binary, string root, string fixture,
    ...
    if (hasAudit) {
        ...
        foreach (canary; ["@example.org", "u0@", "Synthetic public-domain"])
            need(!(cast(string) auditBytes).canFind(canary),
                "audit leaked fixture content");
    }
    return JSONValue([
...
private bool runMatrix(string binary, string root) {
    ...
    foreach (threads; [1, 4]) {
        ...
        auto outHash = hashFile(buildPath(output, "0.txt"));
        auto sideHash = hashFile(buildPath(side, "0.txt.pii-audit.json"));
        if (threads == 1) { firstTree = outHash; firstSide = sideHash; }
        else need(outHash == firstTree && sideHash == firstSide,
            "one/four-thread output mismatch");
    }
    ...
    need(readText(jsonlOut).canFind(`"keep":7`) &&
        !readText(jsonlSide).canFind("scholar@example.org") &&
        !readText(jsonlErr).canFind("scholar@example.org"),
        "JSONL preservation/privacy failed");
    ...
    need(invoke([binary, "run", "--input", equivalentInput, "--output",
        cliOutput, "--sidecar-output", cliAudit, "--threads", "1"] ~
        stageArgs("mask"), root, "equivalent-cli").status == 0 &&
        invoke([binary, "run", "--input", equivalentInput, "--output",
            jsonOutput, "--sidecar-output", jsonAudit, "--threads", "1",
            "--config", jsonConfig], root, "equivalent-json").status == 0 &&
...
private JSONValue handoff(string binary, string root) {
    ...
    auto outputPath = buildPath(root, "handoff-output.jsonl");
    auto errorPath = buildPath(root, "handoff-errors.txt");
    auto auditPath = buildPath(root, "handoff-audit.jsonl");
    write(inputPath, `{"text":"Contact scholar@example.org from 192.0.2.44.\n"}` ~
        "\n");
    {
    ...
    need(!audit.canFind("scholar@example.org") &&
        parseJSON(audit)["output_sha256"].str == hashText(handoffMasked),
        "handoff audit/output mismatch");
    return JSONValue([
        "license": JSONValue("CC0-1.0"),
        "provenance": JSONValue("authored synthetic fixture; no private or network data"),
        "command": JSONValue("printf '%s\\n' '{\"text\":\"Contact scholar@example.org from 192.0.2.44.\\n\"}' | scrubbed run --input - --output - --sidecar-output pii-audit.jsonl --jsonl-fields text --dataset-namespace synthetic-public-v1 --source-key pii-handoff --max-jsonl-line-bytes 4096 --max-jsonl-output-bytes 4096 --stage privacy=pii-four-class --stage-option max-input-bytes=integer:1048576 --stage-option max-findings=integer:4096 --stage-option policy=text:mask"),
        ...
        "expected_audit_utf8": JSONValue(audit),
        "expected_audit_sha256": JSONValue(hashText(audit))]);
}

private void validate(JSONValue report) {
    need(report.type == JSONType.object && report["schema"].str == schema,
        "wrong schema");
    need(digest(report["source_revision"].str) ||
        report["source_revision"].str.length == 40,
    ...
    foreach (entry; metrics) {
        ...
        need(entry["config_sha256"].str == identities[entry["policy"].str ~
            "_config_sha256"].str, "metric/config mismatch");
        if (entry["policy"].str == "disabled")
            ...
            auto prior = entry["fixture"].str in documentByFixture;
            if (prior is null)
                documentByFixture[entry["fixture"].str] = entry["document_id"].str;
            ...
            need(entry["analyzer_sha256"].str == hashText(analyzerIdentity) &&
                entry["policy_sha256"].str == hashText(policyIdentity ~ "/" ~
                    entry["policy"].str), "analyzer/policy identity mismatch");
        }
    }
...
private void mutationControls(JSONValue good) {
    ...
    mustReject(good, (ref JSONValue r) {
        ...
        if (unions.length > 1) {
            ...
            auto contributors = unions[0]["contributors"].array;
            if (contributors.length > 1) {
                auto first = contributors[0];
                contributors[0] = contributors[$ - 1];
                contributors[$ - 1] = first;
            }
        }
        auto text = audit.toString;
        r["handoff"]["expected_audit_utf8"] = JSONValue(text);
...
private JSONValue generate(string binary, string reportPath) {
    ...
    need(runMatrix(binary, root), "actual-binary matrix failed");
    ...
    validate(report);
    mutationControls(report);
    write(reportPath, report.toPrettyString ~ "\n");
    return report;
}

int main(string[] args) {
    if ((args.length == 3 || args.length == 4) && args[1] == "--check") {
        auto report = parseJSON(readText(args[2]));
        validate(report);
        if (args.length == 4)
            need(hashFile(args[3]) ==
