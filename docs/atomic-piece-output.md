...
This is one-destination publication, not a multi-sink transaction. `fsync` covers the temporary file's data before rename, but the parent directory is not synced, so post-crash rename durability is not promised. The destination's previous inode is replaced; hard-link aliases retain the old inode. Ordinary and durable local file/tree jobs consume compiled map/split content through this sink while the mapped owner is live. Durable orchestration records the complete event set first but does not claim multi-file rollback or power-loss atomicity. JSONL uses its stream writer. There is no S3 commit, throughput, or terabyte-readiness claim.
