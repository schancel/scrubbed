...
private enum inputConcatenatedPin =
    "4538A0B393E57FA6EBEE19A7C40FC50E1F6D00FFAE80C8B2424645B8C8938B3C";
private enum outputConcatenatedPin =
    "870D401642B372263AED96C938DE8B2E1E1A466DDCEFA193085889435665A069";
private enum outputBytesPin = 132_579_328L;
...
private JSONValue invoke(string binary, string input, string output,
    ...
    auto dGcStatus = instrumented && ordinal == 0 ?
        (log.canFind("GC summary:") ? "supported" :
            "unsupported-no-summary") : "not-attempted";
    auto result = JSONValue([
        ...
        "d_gc_status": JSONValue(dGcStatus),
...
private void validateMetrics(ref JSONValue sample, size_t expectedFiles,
    ...
    need(phases["discovery"]["calls"].integer == 1 &&
        ...
        "accepted-worker queue accounting differs: " ~ phases.toString);
...
private bool isDigest(string value) {
...
private void validateCommittedReportValue(ref JSONValue report,
        string shippingDigest, string harnessDigest) {
    requireKeys(report, ["schema", "host_os", "host_architecture",
        "host_kernel", "host_cpu", "compiler", "shipping_binary_sha256",
        "harness_sha256", "fixture_table_sha256", "config_sha256",
        "cache_semantics", "production_candidate_authorized", "decision",
        "layouts"], "committed report");
    need(report["schema"].str == "scrubbed.coordination-profile.v1" &&
        !report["production_candidate_authorized"].boolean &&
        report["decision"].str == "ATTRIBUTION_ONLY_NO_CANDIDATE",
        "committed report authority differs");
    foreach (key; ["host_os", "host_architecture", "host_kernel", "host_cpu",
            "compiler"])
        need(report[key].str.length > 0,
            "committed report host identity is empty");
    need(report["cache_semantics"].str ==
        "application-cold; OS cache uncontrolled",
        "committed report cache semantics differ");
    need(report["fixture_table_sha256"].str == fixtureTablePin &&
        report["config_sha256"].str == configPin,
        "committed report fixture identity differs");
    need(isDigest(report["shipping_binary_sha256"].str) &&
        isDigest(report["harness_sha256"].str) &&
        report["shipping_binary_sha256"].str == shippingDigest &&
        report["harness_sha256"].str == harnessDigest,
        "committed report executable identity differs");
    auto reportLayouts = report["layouts"].array;
    need(reportLayouts.length == layouts.length,
        "committed report layout count differs");
    foreach (layoutIndex, ref layout; layouts) {
        auto layoutReport = reportLayouts[layoutIndex];
        requireKeys(layoutReport, ["layout", "files", "input_bytes",
            "input_tree_sha256", "input_concatenated_sha256",
            "performance_samples", "attribution_samples"],
            "committed layout");
        auto expectedBytes = cast(ulong)layout.files * layout.perFile *
            recordBytes;
        need(layoutReport["layout"].str == layout.name &&
            layoutReport["files"].integer == layout.files &&
            layoutReport["input_bytes"].integer == expectedBytes &&
            layoutReport["input_tree_sha256"].str == inputTreePins[layout.name] &&
            layoutReport["input_concatenated_sha256"].str ==
                inputConcatenatedPin,
            "committed layout identity differs");
        foreach (seriesName; ["performance_samples", "attribution_samples"]) {
            auto samples = layoutReport[seriesName].array;
            need(samples.length == runs * 3,
                "committed sample cardinality differs");
            foreach (sampleIndex, ref sample; samples) {
                auto instrumented = seriesName == "attribution_samples";
                requireKeys(sample, instrumented ?
                    ["threads", "ordinal", "wall_us", "user_us", "system_us",
                        "peak_rss_bytes", "sampled_fd_peak", "log_sha256",
                        "stack_status", "stack_sha256", "d_gc_status",
                        "syscall_status", "metrics", "output_bytes",
                        "output_tree_sha256", "output_concatenated_sha256"] :
                    ["threads", "ordinal", "wall_us", "user_us", "system_us",
                        "peak_rss_bytes", "sampled_fd_peak", "log_sha256",
                        "stack_status", "stack_sha256", "d_gc_status",
                        "syscall_status", "output_bytes", "output_tree_sha256",
                        "output_concatenated_sha256"],
                    "committed sample");
                auto expectedThreads = [1, 2, 4][sampleIndex % 3];
                auto wall = sample["wall_us"].integer;
                auto user = sample["user_us"].integer;
                auto system = sample["system_us"].integer;
                auto rss = sample["peak_rss_bytes"].integer;
                auto fd = sample["sampled_fd_peak"].integer;
                need(wall > 0 &&
                    wall <= sampleTimeoutSeconds * 1_000_000,
                    "committed sample wall duration differs");
                auto cpuLimit = wall * (expectedThreads + 2) + 1_000_000;
                need(sample["threads"].integer == expectedThreads &&
                    sample["ordinal"].integer == sampleIndex / 3 &&
                    user > 0 && system >= 0 &&
                    user <= cpuLimit && system <= cpuLimit - user &&
                    rss >= 1_048_576 && rss <= 16L * 1024 * 1024 * 1024 &&
                    fd > 0 && fd <= 4_096 &&
                    isDigest(sample["log_sha256"].str) &&
                    sample["output_bytes"].integer == outputBytesPin &&
                    sample["output_tree_sha256"].str ==
                        outputTreePins[layout.name] &&
                    sample["output_concatenated_sha256"].str ==
                        outputConcatenatedPin,
                    "committed sample identity differs");
                auto stackStatus = sample["stack_status"].str;
                auto stackDigest = sample["stack_sha256"].str;
                if (instrumented && sample["ordinal"].integer == 0)
                    need((stackStatus == "supported" && isDigest(stackDigest)) ||
                        ((stackStatus == "unsupported-sample-failed" ||
                            stackStatus == "unsupported-empty") &&
                            stackDigest.length == 0),
                        "committed stack availability control differs");
                else
                    need(stackStatus == "not-attempted" &&
                        stackDigest.length == 0,
                        "committed stack availability control differs");
                need(sample["syscall_status"].str ==
                    "unsupported-no-exact-child-counter",
                    "committed syscall availability control differs");
                if (instrumented) {
                    auto dGcStatus = sample["d_gc_status"].str;
                    need(sample["ordinal"].integer == 0 ?
                        (dGcStatus == "supported" ||
                            dGcStatus == "unsupported-no-summary") :
                        dGcStatus == "not-attempted",
                        "committed D-GC availability control differs");
                    validateMetrics(sample, layout.files, expectedBytes);
                } else need(sample["d_gc_status"].str == "not-attempted",
                    "performance sample claimed a D-GC control");
            }
        }
    }
}

private alias ReportMutation = void delegate(ref JSONValue);

private void expectCommittedMutationRejected(string reportText,
        string shippingDigest, string harnessDigest, ReportMutation mutate,
        string label) {
    auto mutant = parseJSON(reportText);
    mutate(mutant);
    bool rejected;
    try validateCommittedReportValue(mutant, shippingDigest, harnessDigest);
    catch (Exception) { rejected = true; }
    need(rejected, "committed report mutant was accepted: " ~ label);
}

private void validateCommittedReport(string reportPath, string shippingBinary,
        string harnessPath) {
    need(isFile(reportPath) && !isSymlink(reportPath) &&
        getSize(reportPath) <= diagnosticFileMaxBytes,
        "committed report must be a bounded regular file");
    auto reportText = readText(reportPath);
    auto shippingDigest = fileDigest(shippingBinary);
    auto harnessDigest = fileDigest(harnessPath);
    auto report = parseJSON(reportText);
    validateCommittedReportValue(report, shippingDigest, harnessDigest);
    expectCommittedMutationRejected(reportText, shippingDigest, harnessDigest,
        (ref JSONValue value) { value["decision"] = "AUTHORIZED"; },
        "authority");
    expectCommittedMutationRejected(reportText, shippingDigest, harnessDigest,
        (ref JSONValue value) {
            value["shipping_binary_sha256"] = "NOT-A-DIGEST";
        }, "executable digest");
    expectCommittedMutationRejected(reportText, shippingDigest, harnessDigest,
        (ref JSONValue value) {
            value["layouts"][0]["input_tree_sha256"] = "NOT-A-DIGEST";
        }, "input tree digest");
    expectCommittedMutationRejected(reportText, shippingDigest, harnessDigest,
        (ref JSONValue value) {
            value["layouts"][0]["input_concatenated_sha256"] =
                "NOT-A-DIGEST";
        }, "input concatenated digest");
    expectCommittedMutationRejected(reportText, shippingDigest, harnessDigest,
        (ref JSONValue value) {
            value["layouts"][0]["performance_samples"][0]["output_bytes"] = 0;
        }, "output bytes");
    expectCommittedMutationRejected(reportText, shippingDigest, harnessDigest,
        (ref JSONValue value) {
            value["layouts"][0]["performance_samples"][0]
                ["output_tree_sha256"] = "NOT-A-DIGEST";
        }, "output tree digest");
    expectCommittedMutationRejected(reportText, shippingDigest, harnessDigest,
        (ref JSONValue value) {
            value["layouts"][0]["performance_samples"][0]
                ["output_concatenated_sha256"] = "NOT-A-DIGEST";
        }, "output concatenated digest");
    foreach (field; ["wall_us", "user_us", "system_us", "peak_rss_bytes",
            "sampled_fd_peak"])
        expectCommittedMutationRejected(reportText, shippingDigest,
            harnessDigest, (ref JSONValue value) {
                value["layouts"][0]["performance_samples"][0][field] = -1;
            }, "sample range " ~ field);
    foreach (field; ["user_us", "system_us"])
        expectCommittedMutationRejected(reportText, shippingDigest,
            harnessDigest, (ref JSONValue value) {
                value["layouts"][0]["performance_samples"][0][field] =
                    long.max;
            }, "overflowing CPU " ~ field);
    expectCommittedMutationRejected(reportText, shippingDigest, harnessDigest,
        (ref JSONValue value) {
            auto sample = value["layouts"][0]["performance_samples"][0];
            auto limit = sample["wall_us"].integer *
                (sample["threads"].integer + 2) + 1_000_000;
            value["layouts"][0]["performance_samples"][0]["user_us"] = limit;
            value["layouts"][0]["performance_samples"][0]["system_us"] = 1;
        }, "combined CPU just above limit");
    expectCommittedMutationRejected(reportText, shippingDigest, harnessDigest,
        (ref JSONValue value) {
            value["layouts"][0]["performance_samples"][0]["log_sha256"] =
                "NOT-A-DIGEST";
        }, "log digest");
    expectCommittedMutationRejected(reportText, shippingDigest, harnessDigest,
        (ref JSONValue value) {
            value["layouts"][0]["performance_samples"][0]["stack_status"] =
                "supported";
        }, "stack status and digest");
    expectCommittedMutationRejected(reportText, shippingDigest, harnessDigest,
        (ref JSONValue value) {
            value["layouts"][0]["performance_samples"][0]["d_gc_status"] =
                "supported";
        }, "D-GC status");
    expectCommittedMutationRejected(reportText, shippingDigest, harnessDigest,
        (ref JSONValue value) {
            value["layouts"][0]["performance_samples"][0]["syscall_status"] =
                "supported";
        }, "syscall status");
    writeln("coordination committed evidence: ok");
}
...
private void runSelfTest(string harnessPath) {
    ...
    need(invalidRejected, "zeroed attribution was accepted");
...
void main(string[] args) {
    ...
    need(args.length == 2 || args.length == 3 || args.length == 4,
        ...
        "--validate-report <report> <release-binary> | --self-test");
    if (args.length == 2 && args[1] == "--self-test")
        runSelfTest(absolutePath(args[0]));
    else if (args.length == 4 && args[1] == "--validate-report")
        validateCommittedReport(absolutePath(args[2]), absolutePath(args[3]),
            absolutePath(args[0]));
