...
`verify` repeats the check and runs sixteen release-active missing/corrupt
controls covering the binary, checksum manifest, and every bundled notice. It
rejects symlinks, unexpected files and missing members.
...
`libsqlite3` dependency. A future release pass needs a modern Linux build and
