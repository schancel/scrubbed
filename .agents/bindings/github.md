...
| ready | Maintainer-applied `factory-ready` label + acceptance criteria + owner present; delegated work also names repo, base SHA, authority |
...
- `.agents/scripts/issue_export.py` exports only issues carrying the
  maintainer-controlled `factory-ready` label. Untrusted issue-body text alone
  can never enter the dispatchable queue.
- `.agents/scripts/ticket_triage.py` reads that JSON issue export, prints
  scores, `NEEDS_SPECIFICATION`, and declared-scope conflicts.
